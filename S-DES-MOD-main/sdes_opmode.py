...
def sdes_encrypt_cbc(text: bitarray, key: bitarray, iv:bitarray):
    ...
    for i in range(0,int(len(text) / 8)):
        if i == 0:
            result += sdes(text[i * 8:(i+1) * 8] ^ iv, key, MODE_ENCRYPT)
            prevResult = sdes(text[i * 8:(i+1) * 8] ^ iv, key, MODE_ENCRYPT)
        else:
            result += sdes(text[i * 8:(i+1) * 8] ^ prevResult, key, MODE_ENCRYPT)
            prevResult = sdes(text[i * 8:(i+1) * 8] ^ prevResult, key, MODE_ENCRYPT)
...
def sdes_decrypt_cbc(ciphertext: bitarray, key: bitarray, iv:bitarray):
    ...
    for i in range(0,int(len(ciphertext) / 8)):
        if i == 0:
            result += sdes(ciphertext[i * 8:(i+1) * 8] , key, MODE_DECRYPT) ^ iv
            prevResult = sdes(ciphertext[i * 8:(i+1) * 8] , key, MODE_DECRYPT) ^ iv
        else:
            result += sdes(ciphertext[i * 8:(i+1) * 8] , key, MODE_DECRYPT) ^ prevResult
            prevResult = sdes(ciphertext[i * 8:(i+1) * 8] , key, MODE_DECRYPT) ^ prevResult
