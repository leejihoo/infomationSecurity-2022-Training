...
def sdes(text: bitarray, key: bitarray, mode) -> bitarray:
    ...
    round_keys = []
    xorResult = bitarray()
    roundOneResult = bitarray()
    roundTwoResult = bitarray()
    ...
    for i in IP:
        ...
        if(countIP <= 4):
            ...
            R.append(text[i])
    print("Round1_L: " + str(L[0]) + str(L[1]) + str(L[2]) + str(L[3]) + " " + "Round1_R: " + str(R[0]) + str(R[1]) + str(R[2]) + str(R[3]) )
    # 키 생성"bitarray.bitarray"
    round_keys = schedule_keys(key)
    
    # R 변환
    changedR = round(R,round_keys[0] if mode==1 else round_keys[1])
    print("changedR: " + str(changedR))
    print("round1_round_key: " + str(round_keys[0] if mode==1 else round_keys[1]))
    for i in range(4):
        xorResult.append(L[i] ^ changedR[i])
    #xorResult = xor(L,changedR)
    print("xorResult: " + str(xorResult))
    
    # SW 적용   
    roundOneResult = R + xorResult    
    print("roundOneResult: " + str(roundOneResult))
    ...
    for i in range(8):
        countIP += 1
        if(countIP <= 4):
            L.append(roundOneResult[i])
        else:
            R.append(roundOneResult[i])
    
    # 키 생성
    round_keys = schedule_keys(key)
    
    # R 변환
    changedR = round(R,round_keys[1] if mode==1 else round_keys[0])
    print("round1_round_key: " + str(round_keys[1] if mode==1 else round_keys[0]))
    for i in range(4):
        xorResult.append(L[i] ^ changedR[i])
    #xorResult = xor(L,changedR)

    # SW 적용 안함
    roundTwoResult = xorResult + R

    # IP_1 적용
    for i in IP_1:
        result.append(roundTwoResult[i])
